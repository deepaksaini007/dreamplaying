...
import { TipRequestData } from 'src/app/core/data-models/tip/tip.model';
...
export class AddTipComponent implements OnInit {
  constructor(public dialogRef: MatDialogRef<AddTipComponent>,@Inject(MAT_DIALOG_DATA) public tipRequestData:{tip:TipRequestData|undefined,categoryName:string|undefined},private storeService:StoreService){}
    formData!:FormGroup
  ...
  ngOnInit(): void {
    ...
    this.formData = new FormGroup({
      ...
      tip_title_en:new FormControl(null,[Validators.required]),
      tip_title_hn:new FormControl(null,[Validators.required]),

      tip_decription_en:new FormControl(null,[Validators.required]),
      tip_decription_hn:new FormControl(null,[Validators.required])

    })
      this.listenForCategoryChange()
      if(this.tipRequestData){

        this.formData.patchValue({...this.tipRequestData.tip,category_details:{cat_display_name_en:this.tipRequestData.categoryName}})
        if(this.tipRequestData.tip && !this.tipRequestData.categoryName){
          this.formData.patchValue({category_details:{category_master_id:this.tipRequestData.tip.category_master_id}})
        }
      }
  }
  ...
  submitData(){
    ...
    this.dialogRef.close({
      status:true,
      data: {...this.tipRequestData.tip ,...this.formData.value}
    })
  }
  closeDialog() {
    this.dialogRef.close()
